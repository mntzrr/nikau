...
pub fn init_logging() {
    let filter_layer = EnvFilter::try_from_env("LOG_LEVEL")
        ...
        .add_directive("quinn_proto=info".parse().expect("Failed to parse quinn_proto directive"))
        // x11rb_async: Gets very noisy when copying/pasting things during LOG_LEVEL=trace
        .add_directive("x11rb_async=info".parse().expect("Failed to parse x11rb_async directive"));
